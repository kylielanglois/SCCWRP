...
dat<-read.csv(file.path(path, "2025-07-17 Citrus gPie.csv"))

map.ex<-read_excel(
  file.path("/Users/kylielanglois/SCCWRP/eDNA Sample Management - eDNA/CITRUS_AUTOSAMPLER", 
            "eDNAMasterFile_CITRUS_AUTOSAMPLER.xlsx"), 
                sheet = 2)
map.in<-read_excel(  
  file.path("/Users/kylielanglois/SCCWRP/eDNA Sample Management - eDNA/CITRUS_AUTOSAMPLER", 
            "eDNAMasterFile_CITRUS_AUTOSAMPLER.xlsx"),
...
dat2$sample.unique<-ifelse(grepl("NA ", dat2$sample.unique), 
                           paste(dat2$SampleType.y, dat2$Target, dat2$dilution), 
                           dat2$sample.unique)
#dat2$sample.unique<-paste(dat2$SCCWRP_ID, dat2$Target, 
#                          dat2$Sample.description.3, dat2$dilution) #T grad only

dat2$DateCollected.yymmdd
...
write.csv(dat1.m, file.path(path,
                            "2025-07-17 Citrus gPie_processed.csv"),
          row.names = F)
```
