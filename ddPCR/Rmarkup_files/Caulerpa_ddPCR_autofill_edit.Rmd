---
title: "caulerpa_ddPCR_autofill_edit"
author: "K_Langlois"
date: "`r Sys.Date()`"
...
```{r file, echo = T}
path<-"/Users/kylielanglois/OneDrive - SCCWRP/algae/caulerpa/ddPCR_field"

dat<-read.csv(file.path(path, "2025-06-11 Caulerpa.csv"),
              head=T)
dat1<-dat

#edit and combine metadata---
map.in<-read_excel("/Users/kylielanglois/SCCWRP/eDNA Sample Management - eDNA/CAULERPA/eDNAmasterfile_Caulerpa_inventory_extraction.xlsx", 
                sheet = 2)
map.ex<-read_excel("/Users/kylielanglois/SCCWRP/eDNA Sample Management - eDNA/CAULERPA/eDNAmasterfile_Caulerpa_inventory_extraction.xlsx", 
                   sheet = 3)
map.dd<-read_excel("/Users/kylielanglois/SCCWRP/eDNA Sample Management - eDNA/CAULERPA/eDNAmasterfile_Caulerpa_inventory_extraction.xlsx", 
                   sheet = 4)
...
ntc<-dat1.m[grepl("NTC", dat1.m$sample.unique),
            match(c("sample.unique", "Target", "ddPCR.date",
                    "Positives","Negatives", "Accepted.Droplets"), 
                  colnames(dat1.m))] 
...
#pull out blanks
EB<-dat1.m[grepl("EB|FB|BB", dat1.m$sample.unique),
           match(c("sample.unique", "Target", "ddPCR.date", "Well",
                   "Positives","Negatives", 
                   "Accepted.Droplets"), colnames(dat1.m))] 
paste("blanks positive droplets:", paste0(EB$Positives, collapse = ", ")) 
#check extraction blanks are 0

#pull out positive controls
POS<-dat1.m[grepl("POS", dat1.m$sample.unique),
            match(c("sample.unique", "Target", "Well", "ddPCR.date",
                    "Positives","Negatives", "Accepted.Droplets"), 
                  colnames(dat1.m))] 
...
write.csv(dat1.m, file.path(path,
                            "2025-06-11 caulerpa_processed.csv"),
          row.names = F)
```
