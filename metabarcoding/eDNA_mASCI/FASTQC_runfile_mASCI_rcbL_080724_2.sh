...
#user input-------------------
project_shortname="mASCI_rcbL_080724" #PUT PROJECT SHORT NAME HERE
fastq_directory=$rawPATH/"mASCI_rcbL_080724/fastq_files"	# Name of the folder containing the fastqs. It must be located in the parent directory. Should NOT finish with "/"
...
primerF="AGGTGAAGTAAAAGGTTCWTACTTAAA" #forward primer
primerR="CCTTCTAATTTACCWACWACTG" #reverse primer
...
paste <(echo "") <(echo "50%") <(printf %s "$FASTQC_VAR1_95") <(printf %s "$FASTQC_VAR2_95") <(printf %s "$DADA2_VAR_95")
paste <(echo "") <(echo "50/75%") <(printf %s "$FASTQC_VAR1_95") <(printf %s "$FASTQC_VAR2_99") <(printf %s "$DADA2_VAR_95_99")
paste <(echo "") <(echo "75%") <(printf %s "$FASTQC_VAR1_99") <(printf %s "$FASTQC_VAR2_99") <(printf %s "$DADA2_VAR_99")
paste <(echo "") <(echo "75/85%") <(printf %s "$FASTQC_VAR1_99") <(printf %s "$FASTQC_VAR2_95") <(printf %s "$DADA2_VAR_99_95")
printf "\n"
