...
workingPATH="/" 	# Path to the parent directory. It is not the directory where are stored the fastqs. Should NOT finish with "/"
fastq_directory=$workingPATH/""	# Name of the folder containing the fastqs. It must be located in the parent directory. Should NOT finish with "/"
scriptsPATH="" #where scripts are located; must contain fastqc_per_base_quality_max.R dada2_runfile2.r
primerF="GTGYCAGCMGCCGCGGTAA" #forward primer
primerR="CCGYCAATTYMTTTRAGTTT" #reverse primer
...
paste <(echo "") <(echo "85%") <(printf %s "$Q1_85") <(printf %s "$Q2_85")
...
Rscript --vanilla dada2_runfile2.r ${fastq_directory} ${Q1_85} ${Q2_85}
DADA2_VAR_85=$(sed '2q;d' $fastq_directory/average_quality.txt)
rm $fastq_directory/filtered/*_filt.fastq.gz
rmdir $fastq_directory/filtered

Rscript --vanilla dada2_runfile2.r ${fastq_directory} ${Q1_85} ${Q2_75}
...
Rscript --vanilla dada2_runfile2.r ${fastq_directory} ${Q1_75} ${Q2_50}
...
paste <(echo "") <(echo "85%")  <(printf %s "$DADA2_VAR_85")
paste <(echo "") <(echo "85/75%")  <(printf %s "$DADA2_VAR_85_75")
paste <(echo "") <(echo "75%")  <(printf %s "$DADA2_VAR_75")
paste <(echo "") <(echo "75/50%")  <(printf %s "$DADA2_VAR_75_50")
